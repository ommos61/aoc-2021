    ...
    for (int i = 0; i < s1->beacon_count; i++) {
        for (int j = 0; j < s2->beacon_count; j++) {
            ...
            if (match_count >= 12) {
                ...
                s2->pos->x = s1->pos->x + diff.x;
                s2->pos->y = s1->pos->y + diff.y;
                s2->pos->z = s1->pos->z + diff.z;
                return match_count;
            } else {
...
// Maximum number of beacons per scanner
#define BEACON_MAX 30
    point *unique_beacons = malloc(scanner_count * BEACON_MAX * sizeof(point));
    int beacon_count = 0;
    for (int i = 0; i < scanner_count; i++) {
        for (int j = 0; j < scanners[i].beacon_count; j++) {
            assert(unique_beacons != NULL);
            int already_found = 0;
            for (int k = 0; k < beacon_count; k++) {
                if (pointsEqual(&unique_beacons[k], &scanners[i].beacons[j])) {
                    already_found = 1;
                    break;
                }
            }
            if (! already_found) {
                unique_beacons[beacon_count++] = scanners[i].beacons[j];
            }
        }
    }
    printf("Unique number of beacons: %d\n", beacon_count);
