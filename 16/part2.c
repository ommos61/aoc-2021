...
long performOperation(int op, long val1, long val2) {
    long result = val1;
...
int sum_versions = 0;
int parseLine(const char *input, int offset, long *answer) {
    int index = offset;
        ...
        *answer = val;
        ...
        if (len_type == 0) {
            ...
            while (new_index < index + len) {
                long value = 0;
                new_index = parseLine(input, new_index, &value);
                result = performOperation(type_id, result, value);
            }
            ...
            *answer = result;
            ...
            for (int i = 0; i < cnt; i++) {
                long value = 0;
                index = parseLine(input, index, &value);
                result = performOperation(type_id, result, value);
            }
            *answer = result;
        }
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < line_count; i++) {
        ...
        sum_versions = 0;
        answer = 0;
        int index = parseLine(lines[i], 0, &answer);
        printf("----> Answer: %ld\n", answer);
